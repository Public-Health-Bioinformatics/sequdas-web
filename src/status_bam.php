...
<div id="page-wrapper">
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            $db2 = DB2::getInstance(); 
            $sql='SELECT * FROM status_table where source='.'"BAM"';
            $query_miseqs = $db2->query($sql);
            $count = $query_miseqs->count();
            $sql = 'SELECT * FROM status_table where source='.'"BAM"'.' LIMIT ' . (($pagination->get_page() - 1) * $records_per_page) . ', ' . $records_per_page . '';
            $query_miseqs = $db2->query($sql);
